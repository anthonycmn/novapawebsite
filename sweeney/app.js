/* Sweeney Todd: School Edition staff dashboard (cloned from deh/).
   Schedule data: sweeney/data.js. Scene breakdown: sweeney/scenes.js. Neither is
...
(function () {
  var D = window.SWEENEY, S = window.SWEENEYSCENES;
  var LS_DONE = 'swy.done.v1', LS_ITEM = 'swy.items.v1', LS_ME = 'swy.me.v1', LS_GATE = 'swy.gate.v1';
  var LS_ROSTER = 'swy.roster.v1', LS_ATT = 'swy.att.v1', LS_NOTE = 'swy.notes.v1', LS_REP = 'swy.rep.v1';
  var LS_STRIKE = 'swy.strike.v1';
  var GATE_WORD = 'fleet';   // curtain, not a lock — change here and tell staff

  // Who can sign a block off. Anyone not listed picks "Someone else" and types.
  var STAFF = ['Katie H.', 'Choreographer', 'Assistant Director', 'Tony', 'Stage Manager'];
  ...
  var TRACKS = {
    ...
    'ROOM A': 'music', 'ROOM B': 'stage', 'ROOM C': 'study', 'JOINT': 'co',
    'DRESS': 'perf', 'SITZPROBE': 'music'
  };
  ...
  var ACCESS = [
    ...
    { k: 'MTI ACCOUNT', v: 'Northern Virginia Performing Arts, account number 9012523. The Sweeney Todd School Edition licence sits under this account — booking and materials order numbers go here once Todd confirms them.' },
    { k: 'THE SCHOOL EDITION', v: 'The script and score cannot be altered — that is the licence. Trust its cuts and do not add material back. Confirm with MTI that the silent Young Lucy figure is permitted BEFORE she is cast.' },
    { k: 'SCORES AND VOCAL BOOKS', v: 'From MTI with the licence. Every music call on the schedule needs an accompanist booked — twenty-plus calls across the eight weeks.' },
    { k: 'ORG LOGIN', v: 'Held by CJ. Not printed here because this page lives in a public repository and our MTI licence does not permit publishing account credentials.' },
    { k: 'PLAYBACK RULES', v: 'If MTI Player tracks are ordered: once the show is loaded on a device, do not update the app or iOS until we close. Airplane mode with WiFi off before every playback — a call or auto-update will interrupt the track mid-number.' },
    { k: 'BAND DECISION', v: 'Full, reduced, or two-piano — decide by Sep 1. It moves the budget and the pit footprint more than anything else in the show. The Beadle also needs a playable harmonium for Parlor Songs by Sep 18.' },
    { k: 'IF SOMETHING BREAKS', v: 'MTI Player support: (877) 845-4704 or player@mtishows.com. Licensing: (212) 541-4684. Mid-performance emergency hotline: (888) 340-3505 — for a track failing during a show, nothing else.' }
  ];
  ...
  // Until casting is published, attendance still has to work. One row per
  // School Edition principal track, unnamed — names get typed in beside a
  // role as students are cast, and they save like anything else.
  var SEED_VERSION = 1;
  var ROLES = [
    'Sweeney Todd', 'Mrs. Lovett', 'Anthony Hope', 'Johanna Barker',
    'Tobias Ragg', 'Judge Turpin', 'Beadle Bamford', 'Beggar Woman',
    'Adolfo Pirelli', 'Jonas Fogg', 'Young Lucy'
  ];
  var CREATIVE = [
    { person_id: 'staff-katieh', name: 'Katie H.', role: 'Director / Vocal Director', kind: 'staff', sort: 1 },
    { person_id: 'staff-choreo', name: '', role: 'Choreographer', kind: 'staff', sort: 2 },
    { person_id: 'staff-ad', name: '', role: 'Assistant Director (Saturdays)', kind: 'staff', sort: 3 },
    { person_id: 'staff-tony', name: 'Tony Cimino-Johnson', role: 'Producer', kind: 'staff', sort: 4 }
  ];
  function castRows() {
    return ROLES.map(function (r, i) {
      return { person_id: 'cast-' + slug(r), name: '', role: r, kind: 'cast', sort: 10 + i };
    });
  }
  ...
  // Every database call goes through /api/sweeney-db. The browser holds no key:
  ...
  function renderDay() {
    ...
    var staff = [['Katie H.', cov.katie], ['Choreographer', cov.choreo],
                 ['Asst. Director', cov.ad], ['Tony', cov.tony]]
  ...
  function reportPlainText(r) {
    var L = ['REHEARSAL REPORT — Sweeney Todd: School Edition', r.dayLabel + (r.code ? ' (' + r.code + ')' : ''),
  ...
  function buyListText() {
    ...
    var L = ['SWEENEY TODD — still to buy', ''];
