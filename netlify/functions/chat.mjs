...
ACTIVE DISCOUNT (through August 15, 2026, applied automatically at checkout): 10% off any camp, 15% off each camp when one camper registers for two camps, 20% off each when one camper does three — the multi-camp tiers are per camper, not across siblings. Families still add every child's camps to one cart and check out once (one free family account covers the whole household). SHOW + CAMP COMBO: a camper who pairs any summer camp with one of the school-year shows (Frozen or The Little Mermaid) gets 15% off ALL of their registrations; a camp plus BOTH shows is 20% off everything. Each age group has limited slots — encourage early registration. Campers can do one, two, or all three shows.
