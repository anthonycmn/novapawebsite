...
// Resend first (the novapa.org domain is verified there for receipts); if
// Resend is not configured or refuses the send, Gmail SMTP carries the same
// message so a family is never left without their confirmation.
...
async function sendConfirmation(b, cls) {
  const subject = `${b.child_name}'s free class is booked`;
  const html = confirmationHtml(b, cls);
  if (process.env.RESEND_API_KEY) {
    try {
      const r = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ from: CONFIRM_FROM, reply_to: CONFIRM_REPLY_TO, to: [b.email], bcc: CONFIRM_BCC, subject, html }),
      });
      if (r.ok) return;
      console.error(`freeclass confirmation via Resend refused ${r.status}: ${(await r.text()).slice(0, 200)}`);
    } catch (e) {
      console.error("freeclass confirmation via Resend failed:", e.message);
    }
  }
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) throw new Error("no mail transport configured");
